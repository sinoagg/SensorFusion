...
uint8_t ARS_ConfigFilter(CAN_HandleTypeDef *hcan)
{
  ...
  FilterContent.FilterCfg_Min_Azimuth = (uint16_t)((-50 + 50) / 0.025);  //-9°~9°，offset -50, Res 0.025
  ...
  FilterContent.FilterCfg_Min_Lifetime = (uint16_t)((0 - 0) / 0.1);   //0.1~409.5s, offset 0, Res 0.1
